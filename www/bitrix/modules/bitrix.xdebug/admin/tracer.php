...
elseif ($_REQUEST['mode'] == 'ajax')
{
	...
	if (src_iframe = document.getElementById('src_iframe'))
	{
		if (ob = document.getElementById('fileNav'))
		{
			...
			filesize = <?=$filesize?>;
			p = <?=$p?>;
			html = '<div><?=GetMessageJS("BITRIX_XDEBUG_POZICIA_V_TREYSE")?><?=$XTR->pos?> / ' + filesize + ' (<?=HumanSize($XTR->pos)?> / <?=HumanSize($filesize)?>) - ' + p + '%</div>';
			for (i = 1; i<=100; i++)
			...
			ob.innerHTML = "<table><? 
				for($i = 1; $i<=$XTR->depth; $i++)
				{
					...
					echo '<tr valign=\"top\">'.
						...
						'<td style=\"color:#9c6\">'.($XRec0->mem > 0 ? HumanSize($XRec0->mem) : '').'</td>'.
						'<td><nobr>'.str_repeat('&nbsp;',$i).' &gt; '.$strFunc."</nobr><br>".$strResult.'</td>'.
					'</tr>';
				}
				?></table>";
...
$APPLICATION->SetTitle(GetMessageJS("BITRIX_XDEBUG_OTLADKA").': '.$trace.' ('.HumanSize(filesize($trace_output_dir.'/'.$trace)).') '.GetMessageJS("BITRIX_XDEBUG_OT").date('d.m.Y H:i:s', filemtime($trace_output_dir.'/'.$trace)));
...
<script>
	...
	function MakeStep(search_depth, text_search_file, text_search_func, text_search_var)
	{
		...
		if (text_search_func)
			req += '&text_search_func=' + encodeURIComponent(text_search_func);
		if (text_search_var)
...
<?=GetMessage("BITRIX_XDEBUG_POISK_FAYLA")?><input size=20 title="<?=GetMessage("BITRIX_XDEBUG_POISK_PO_PODSTROKE_P")?>" id="text_search_file" onkeydown="if(event.keyCode == 13)SearchText()"> 
	<?=GetMessage("BITRIX_XDEBUG_FUNKCII")?><input size=10 title="<?=GetMessage("BITRIX_XDEBUG_POISK_S_UCETOM_REGIS")?>" id="text_search_func" onkeydown="if(event.keyCode == 13)SearchText()"> 
