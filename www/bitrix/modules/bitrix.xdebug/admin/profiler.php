...
if ($_REQUEST['mode'] == 'ajax')
{
	...
	$m = new CAdminMessage(array(
		...
		"DETAILS" => '#PROGRESS_BAR#'.' '.GetMessage("BITRIX_XDEBUG_POZICIA").HumanSize($XProfile->pos).' '.GetMessage("BITRIX_XDEBUG_IZ").HumanSize($filesize).', '.GetMessage("BITRIX_XDEBUG_OSTALOSQ_PRIMERNO").sprintf('%02d:%02d:%02d', floor($time_left / 3600), floor($time_left % 3600 / 60), $time_left % 60),
		"HTML" => true,
...
else
{
	...
	$APPLICATION->SetTitle(GetMessage("BITRIX_XDEBUG_PROFILIROVANIE").': '.$trace.' ('.HumanSize(filesize($trace_output_dir.'/'.$trace)).') '.GetMessage("BITRIX_XDEBUG_OT").date('d.m.Y H:i:s', filemtime($trace_output_dir.'/'.$trace)));
	require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
