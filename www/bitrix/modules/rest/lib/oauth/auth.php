	...
	public static function onRestCheckAuth(array $query, $scope, &$res)
	{
		...
		if($authKey)
		{
			...
			if(is_array($tokenInfo))
			{
				...
				{
					$tokenInfo = array('error' => 'expired_token');
					$error = true;
				}
				...
				if(!$error && $scope !== \CRestUtil::GLOBAL_SCOPE && isset($tokenInfo['scope']))
				{
					...
					{
						$tokenInfo = array('error' => 'insufficient_scope');
						$error = true;
					}
				}
				...
				if(!$error && $tokenInfo['user_id'] > 0)
				{
					...
					{
						$tokenInfo = array('error' => 'authorization_error');
						$error = true;
					}
					elseif(!\CRestUtil::checkAppAccess($tokenInfo['client_id']))
					{
						$tokenInfo = array('error' => 'user_access_error');
						$error = true;
					}
				}
