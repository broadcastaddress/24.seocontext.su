<?php
namespace Bitrix\ImBot;

class Controller
{
	public static function sendToBot($botName, $command, $params)
	{
		$result = null;

		$botName = trim(preg_replace("/[^a-z]/","", strtolower($botName)));
		if (!$botName)
			return $result;

		foreach ($params as $key => $value)
		{
			if ($value == '#ZERO#')
				$value = '0';
			else if ($value == '#EMPTY#')
				$value = '';

			$params[$key] = $value;
		}

		if (class_exists('\\Bitrix\\ImBot\\Bot\\'.ucfirst($botName)) && method_exists('\\Bitrix\\ImBot\\Bot\\'.ucfirst($botName), 'onAnswerAdd'))
		{
			return call_user_func_array(array('\\Bitrix\\ImBot\\Bot\\'.ucfirst($botName), 'onAnswerAdd'), Array($command, $params));
		}
		return $result;
	}

	public static function sendToService($serviceName, $command, $params)
	{
		$result = null;

		$serviceName = trim(preg_replace("/[^a-z]/","", strtolower($serviceName)));
		if (!$serviceName)
			return $result;

		foreach ($params as $key => $value)
		{
			if ($value == '#ZERO#')
				$value = '0';
			else if ($value == '#EMPTY#')
				$value = '';

			$params[$key] = $value;
		}

		if (class_exists('\\Bitrix\\ImBot\\Service\\'.ucfirst($serviceName)) && method_exists('\\Bitrix\\ImBot\\Service\\'.ucfirst($serviceName), 'onMessageReceive'))
		{
			return call_user_func_array(array('\\Bitrix\\ImBot\\Service\\'.ucfirst($serviceName), 'onMessageReceive'), Array($command, $params));
		}
		return $result;
	}
}