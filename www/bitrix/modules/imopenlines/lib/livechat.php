...
class LiveChat
{
	...
	private function getChatForUser()
	{
		...
		if($chat = $orm->fetch())
		{
			...
			{
				$chat = new \CIMChat(0);
				$chat->AddUser($this->chat['ID'], $this->userId, false, true); // TODO security context
			}
			return $this->chat;
		}
		...
		if ($this->userId)
		{
			...
			if ($user = $orm->fetch())
			{
				...
				$userName = \Bitrix\Im\User::getInstance($this->userId)->getFullName();
	...
	public static function onMessageSend($messageId, $messageFields)
	{
		...
		Log::write($messageFields, 'LIVECHAT');
		...
		if ($messageFields['AUTHOR_ID'] > 0 && $userId != $messageFields['AUTHOR_ID'])
		{
	...
	private function getGuestUser($userId = null)
	{
		...
		{
			$userName = '';
			$userLastName = Loc::getMessage('IMOL_LC_GUEST_NAME');
		}
		...
		if ($userId && \Bitrix\Im\User::getInstance($userId)->isExists())
		{
			...
			$userName = htmlspecialcharsback($userData->getName());
			$userLastName = htmlspecialcharsback($userData->getLastName());
			$userGender = $userData->getGender();
			$userWorkPosition = htmlspecialcharsback($userData->getWorkPosition());
