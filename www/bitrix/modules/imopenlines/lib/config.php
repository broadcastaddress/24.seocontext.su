...
class Config
{
	...
	private function prepareFields($params, $mode = self::MODE_ADD)
	{
		...
		{
			$fields['NO_ANSWER_TEXT'] = Loc::getMessage('IMOL_CONFIG_NO_ANSWER');
		}
		...
		{
			$fields['WORKTIME_DAYOFF_TEXT'] = Loc::getMessage('IMOL_CONFIG_WORKTIME_DAYOFF_2');
		}
		...
		{
			$fields['CLOSE_TEXT'] = Loc::getMessage('IMOL_CONFIG_CLOSE_TEXT');
		}
	...
	public function get($id, $withQueue = true)
	{
		$id = intval($id);
		if (!$id)
			return false;
