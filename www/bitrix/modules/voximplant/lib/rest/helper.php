...
class Helper
{
	...
	public static function registerExternalCall(array $fields)
	{
		...
		$phoneNumber = \CVoxImplantPhone::Normalize($fields['PHONE_NUMBER'], 0);
		if(!$phoneNumber)
		{
		...
		else if($crmCreate && $isCrmAvailable)
		{
			...
			$newCall['CRM_ACTIVITY_ID'] = \CVoxImplantCrmHelper::AddCall(array(
				'CALL_ID' => $callId,
				'PHONE_NUMBER' => $phoneNumber,
				'INCOMING' => $fields['TYPE'],
				'USER_ID' => $fields['USER_ID'],
	...
	public static function showExternalCall(array $params)
	{
		...
		$phoneNumber = (string)$call['CALLER_ID'];
		$incoming = $call['INCOMING'];

		$crmData = \CVoxImplantCrmHelper::GetDataForPopup($callId, $phoneNumber, $userId);
	...
	public static function getRestAppName($clientId)
	{
		...
		$row = AppTable::getList(array(
			'select' => array(
				'APP_NAME' => 'APP_NAME',
				'MENU_NAME' => 'LANG.MENU_NAME',
				'DEFAULT_MENU_NAME' => 'LANG_DEFAULT.MENU_NAME'),
			'filter' => array(
				'=CLIENT_ID' => $clientId
			)
		))->fetch();
	...
	public static function getExternalCallHandlers()
	{
		...
		$cursor = EventTable::getList(array(
			...
			'filter' => array(
				'EVENT_NAME' => \CVoxImplantRestService::EVENT_START_EXTERNAL_CALL
			)
		));
