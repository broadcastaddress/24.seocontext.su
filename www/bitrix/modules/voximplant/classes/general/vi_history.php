...
class CVoxImplantHistory
{
	public static function Add($params)
	{
		...
		{
			$queueUser = VI\QueueTable::getList(Array(
	 ...
	 * @return string
	 */
	public static function GetMessageForChat($callFields, $hasRecord = false)
	{
		$result = '';
		if (strlen($callFields["PHONE_NUMBER"]) > 0 && $callFields["PORTAL_USER_ID"] > 0 && $callFields["CALL_FAILED_CODE"] != 423)
		{
			$plusSymbol = strlen($callFields["PHONE_NUMBER"]) >= 10 && substr($callFields["PHONE_NUMBER"], 0, 1) != '+' ? '+' : '';
