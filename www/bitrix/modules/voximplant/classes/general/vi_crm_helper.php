...
class CVoxImplantCrmHelper
{
	...
	public static function AddCall($params)
	{
		...
		{
			$crmEntity = array(
				'ENTITY_TYPE_NAME' => $call['CRM_ENTITY_TYPE'],
				'ENTITY_TYPE' => CCrmOwnerType::ResolveID($call['CRM_ENTITY_TYPE']),
				'ENTITY_ID' => $call['CRM_ENTITY_ID'],
				'BINDINGS' => array(
					0 => array(
			...
			$deals = self::findDealsByEntity($call['CRM_ENTITY_TYPE'], $call['CRM_ENTITY_ID']);
		...
		if($params['INCOMING'] === CVoxImplantMain::CALL_CALLBACK)
		{
			$arFields['PROVIDER_TYPE_ID'] = Provider\Call::ACTIVITY_PROVIDER_TYPE_CALLBACK;
			$arFields['SUBJECT'] = Loc::getMessage('VI_CRM_CALLBACK_TITLE');
		}
		else
		{
			$arFields['PROVIDER_TYPE_ID'] = Provider\Call::ACTIVITY_PROVIDER_TYPE_CALL;
			$arFields['SUBJECT'] = Loc::getMessage('VI_CRM_CALL_TITLE');
			$arFields['DIRECTION'] = $direction;
		}
		...
		$activityId = CCrmActivity::Add($arFields, false, true, array('REGISTER_SONET_EVENT' => true));
		if($activityId > 0)
		{
			CCrmActivity::SaveCommunications($activityId, $arComms, $arFields, true, false);
			\Bitrix\Crm\Integration\Channel\VoxImplantTracker::getInstance()->registerActivity($activityId);
			CVoxImplantHistory::WriteToLog($arFields, 'CREATED CRM ACTIVITY '.$activityId);
			return $activityId;
		}
		else
		{
	...
	{
		CVoxImplantOutgoing::startCallBack(
			$params['CALL_FROM'],
			$params['CALL_TO'],
			$params['TEXT'],
