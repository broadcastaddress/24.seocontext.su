...
class CVoxImplantOutgoing
{
	...
	public static function Init($params)
	{
		...
		if ($callAdd)
		{
			$addResult = VI\CallTable::add(Array(
				'CONFIG_ID' => $params['CONFIG_ID'],
				'CALL_ID' => $params['CALL_ID'],
				'CRM' => $params['CRM'],
				'CRM_ENTITY_TYPE' => ($params['CRM_ENTITY_TYPE'] ? $params['CRM_ENTITY_TYPE'] : null),
		...
		if ($params['CRM'] == 'Y' && $result['PORTAL_CALL'] == 'N')
		{
			if ($config['CRM_CREATE'] == CVoxImplantConfig::CRM_CREATE_LEAD)
			{
				...
				if ($crmData['FOUND'] == 'N')
				{
					...
					if($leadId && $callId)
					{
						VI\CallTable::update($callId, array(
	...
	public static function StartCall($userId, $phoneNumber, $params)
	{
		...
		{
			$callFields['CRM_ENTITY_TYPE'] = $params['ENTITY_TYPE'];
		...
		$call = VI\CallTable::add($callFields);
