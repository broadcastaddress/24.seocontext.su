...
(function (VoxImplant, undefined) {
  ...
  VoxImplant.ZingayaAPI = function (videoEnabled, micRequired) {
    ...
    this.setAudioConstraints = function (constraints, successCallback, failedCallback, apply) {
    ...
    this.requestMedia = function (video, onMediaAccessGranted, onMediaAccessRejected, stopStream) {
      var constraints,
        constraintName = ((typeof navigator.mediaDevices != "undefined")
        && (typeof navigator.mediaDevices.enumerateDevices != "undefined")
        && (typeof MediaStreamTrack != "undefined")
        && (typeof MediaStreamTrack.getSources == "undefined")) ? "deviceId" : "sourceId";
      ...
      if (audioSourceId === null && videoSourceId === null) {
        ...
        if (audioSourceId !== null && videoSourceId !== null) {
          if (constraintName == "sourceId") constraints = {
            audio: {mandatory: {sourceId: audioSourceId}},
            video: {mandatory: {sourceId: videoSourceId}}
          };
          else constraints = {
            audio: {
          ...
          if (constraintName == "sourceId") constraints = {audio: true, video: {mandatory: {sourceId: videoSourceId}}};
          else constraints = {
            audio: true,
            video: {
              deviceId: {exact: videoSourceId}
      ...
      if (videoConstraints !== null) {
        ...
        if (typeof videoConstraints.optional != "undefined") {
          ...
          if (videoSourceId !== null) {
            ...
            else constraints.video.deviceId = {exact: videoSourceId};
          }
        } else {
          if (videoSourceId !== null) {
            if (constraintName == "sourceId") constraints.video.mandatory.sourceId = videoSourceId;
      ...
      if (audioConstraints !== null) {
        if (isChrome) {
          ...
          if(audioSourceId !== null) {
            if (constraintName == "sourceId") {
              if (typeof constraints.audio.mandatory == "undefined"){
                constraints.audio.mandatory = {sourceId: audioSourceId}
              }else{
                constraints.audio.mandatory.sourceId = audioSourceId;
              }
            }else {
              constraints.audio.deviceId = {exact: audioSourceId};
            }
          }
        }
      }
...
//					});
      ...
      this.start = function (sdpOffer) {
        ...
        pc.setRemoteDescription(activeRemoteSD, function () {
          pc.createAnswer(function (sdp) {
            addBandwidthParams(sdp);
            BXIM.webrtc.phoneLog("Local answer created: " + sdp.sdp);
            localRTCPMux = sdp.sdp.indexOf("a=rtcp-mux") != -1;
            pc.setLocalDescription(sdp, function () {
              activeLocalSD = sdp;
//                            if (sdp.sdp.indexOf("candidate") != -1) {
...
VoxImplant.Client = function() {
	...
	*/
	this.__init = function(config) {
		...
		if (typeof this.config.audioConstraints != 'undefined') this.audioConstraints = this.config.audioConstraints;
		else this.audioConstraints = null;
	...
	connectTo: function(host, omitMicDetection, connectivityCheck) {
		...
		if (this.RTCsupported && !this.useFlashOnly) { 
			if (!this.micRequired || omitMicDetection === true) this.zingayaAPI.connectTo(host, "platform", null, null, this.connectivityCheck); 
				...
				if (this.audioConstraints) this.zingayaAPI.setAudioConstraints(this.audioConstraints, null, null, false);
...
VoxImplant.Utils = {
	...
	*/
	getServers: function(callback, reservedBalancer, vi) {
		...
		if (reservedBalancer === true) balancer_url = protocol + "balancer2.voximplant.com/getNearestHost";
...
*/
VoxImplant.version = VoxImplant.Client.prototype.version = "3.6.344";
