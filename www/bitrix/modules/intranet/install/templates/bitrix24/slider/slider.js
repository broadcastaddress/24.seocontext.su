...
BX.Bitrix24.PageSlider = {
	...
	closeSkeleton: function(immediately)
	{
		if (immediately === true)
		{
		...
		var sliderPopupLoader = new BX.easing({
			duration: 500,
			start: { opacity: 100},
			finish: { opacity: 0 },
			transition: BX.easing.makeEaseOut(BX.easing.transitions.quart),
			step: BX.proxy(function (state) {
				this.loader.style.opacity = state.opacity / 100;
			}, this),
	...
	extractHandledUrl: function(event)
	{
		...
		if (href && aTarget !== "_top" && aTarget !== "_blank" && !BX.data(a, "slider-ignore-autobinding"))
		{
			var rule = this.getUrlRule(href);
			if (rule !== null && !this.hasStopParams(href, rule.stopParameters))
			{
				return href;
			}
		}
	...
	{
		var url = this.extractHandledUrl(event);
	...
	getUrlRule: function(href)
	{
		...
		for (var k = 0; k < ar.length; k++)
		{
			...
			for (var m = 0; m < rule.condition.length; m++)
			{
				...
				{
					rule.condition[m] = new RegExp(rule.condition[m]);
				}

				if (href.match(rule.condition[m]))
				{
					return rule;
				}
			}
	...
	bindAnchors: function(parameters)
	{
		...
		{
			this.defaultOptions.anchorRules = parameters.rules;
		}

		if(!this.anchorHandler)
		{
			this.anchorHandler = window.document.addEventListener('click', this.handleClick.bind(this), true);
		}
	},
