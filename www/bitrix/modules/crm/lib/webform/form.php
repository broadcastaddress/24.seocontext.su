...
class Form
{
					...
					foreach($field['ITEMS'] as $item)
					{
						$preparedField['items'][] = array(
							...
							'price_formatted' => $item['PRICE'] ?
								\CCrmCurrency::MoneyToString($item['PRICE'], $currencyId)
								:
								null
						);
					}
	...
	public function addResult($resultFields, $resultParameters = array())
	{
		...
		else
		{
			...
			if($fieldPhoneValue && $this->params['IS_CALLBACK_FORM'] == 'Y')
			{
				if(Loader::includeModule('voximplant') && count(\CVoxImplantConfig::GetPortalNumbers(true)) > 0)
				{
					$callEvent = new Event(
						'crm',
	...
	public function sendScriptsEmail($email)
	{
		...
		$this->arResult = array(
			...
			'LINK' => str_replace('#form_id#', $formData['ID'], $linkPath),
			'SCRIPTS' => array(
				'INLINE' => $script->getInline($scriptParams),
				'BUTTON' => $script->getButton($scriptParams + array('button_caption' => Loc::getMessage('CRM_WEBFORM_FORM_SCRIPT_BUTTON_TEXT'))),
				'LINK' => $script->getLink($scriptParams + array('button_caption' => Loc::getMessage('CRM_WEBFORM_FORM_SCRIPT_BUTTON_TEXT'))),
		...
		$fields = array(
			'SCRIPT_DELAY' => $this->params['RESULT_SUCCESS_TEXT'],
			'SCRIPT_INLINE' => $this->params['RESULT_SUCCESS_URL'],
			'SCRIPT_' => $this->params['RESULT_FAILURE_TEXT'],
			'RESULT_FAILURE_URL' => $this->params['RESULT_FAILURE_URL'],
			'FORM_CAPTION' => $this->params['CAPTION'],
