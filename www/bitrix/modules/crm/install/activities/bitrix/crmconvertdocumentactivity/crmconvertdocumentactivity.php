...
class CBPCrmConvertDocumentActivity extends CBPActivity
{
	...
	public function Execute()
	{
		...
		$wizard = $this->getConversionWizard();
	...
	private function getConversionWizard()
	{
		$wizard = null;
		$documentId = $this->GetDocumentId();
		...
		if ($documentId[1] == 'CCrmDocumentLead')
		{
			...
			$wizard = new LeadConversionWizard($this->getEntityId(), $this->getLeadConversionConfig());
		}
		elseif ($documentId[1] == 'CCrmDocumentDeal')
		{
			DealConversionWizard::remove($this->getEntityId());
			$wizard = new DealConversionWizard($this->getEntityId(), $this->getDealConversionConfig());
		}

		return $wizard;
	}

	private function getLeadConversionConfig()
	{
		...
		{
			$item = $config->getItem(\CCrmOwnerType::ResolveID($itemName));
			if (!$item)
				continue;
	...
	private function getDealConversionConfig()
	{
