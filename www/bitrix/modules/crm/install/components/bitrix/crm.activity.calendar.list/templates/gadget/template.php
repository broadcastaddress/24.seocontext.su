...
else
{
	...
	?><script type="text/javascript"><?
		foreach($arResult['TASK'] as $arEvent):
			...
			?>BX.tooltip(<?=$arTask['ENTITY_ID']?>, "balloon_<?=$arResult['GRID_ID']?>_T_<?=$arTask['ID']?>", "/bitrix/components/bitrix/crm.<?=strtolower($arTask['ENTITY_TYPE'])?>.show/card.ajax.php", "crm_balloon<?=($arTask['ENTITY_TYPE'] == 'LEAD' || $arTask['ENTITY_TYPE'] == 'DEAL'? '_no_photo': '_'.strtolower($arTask['ENTITY_TYPE']))?>");<?
			endif;
		endforeach;
	?></script><?
}
