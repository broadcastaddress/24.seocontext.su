...
if (!$USER->IsAuthorized() || $CCrmContact->cPerms->HavePerm('CONTACT', BX_CRM_PERM_NONE))
	die();
...
if ($_REQUEST['MODE'] == 'ENTITY')
{
	...
	if ($SECTION_ID == 'last')
	{
		...
		$arLastSelected = CUserOptions::GetOption("crm", "crm_contact_search", array());
	...
	$dbRes = CCrmContact::GetList(Array('LAST_NAME'=>'ASC', 'NAME' => 'ASC'), $arFilter);
	while ($arRes = $dbRes->Fetch())
	{
		...
		$arFilterCompany['ID'][] = $arRes['COMPANY_ID'];
	}
	$dbRes = CCrmCompany::GetList(Array('TITLE'=>'ASC', 'LAST_NAME'=>'ASC', 'NAME' => 'ASC'), $arFilterCompany);
	while ($arRes = $dbRes->Fetch())
		$arCompanyName[$arRes['ID']] = $arRes['TITLE'];
		
	foreach($arUsers as $key => $ar)
		$arUsers[$key]['COMPANY_NAME'] = intval($ar['COMPANY_ID']) > 0 && isset($arCompanyName[$ar['COMPANY_ID']]) ? htmlspecialchars($arCompanyName[$ar['COMPANY_ID']]): GetMessage('CRM_ES_COMPANY_NONE');
...
?>
				...
				obUserRow.innerHTML = '<div class="bx-entity-photo bx-no-photo"></div><div class="bx-entity-info"><div class="bx-entity-name">' + arEntity[i].NAME + '</div><div class="bx-entity-position">' + arEntity[i].COMPANY_NAME + '</div></div>';
...
function BXLoadEntity(SECTION_ID, bShowOnly, bScrollToSection)
{
	...
	{
		var url = '/bitrix/components/bitrix/crm.entity.search/contact.ajax.php?lang=<?echo LANGUAGE_ID?>&MODE=ENTITY&SECTION_ID=' + SECTION_ID + '&SITE_ID=<?=$SITE_ID?>';
		if (bScrollToSection)
		{
...
var jsEntSearch = {
	...
	Run: function()
	{
		...
		if (jsEntSearch._control.value && jsEntSearch._control.value != jsEntSearch._control.value_tmp)
		{
			...
			jsUtils.loadJSFile('/bitrix/components/bitrix/crm.entity.search/contact.ajax.php?lang=<?echo LANGUAGE_ID?>&MODE=SEARCH&search=' + encodeURIComponent(jsEntSearch._value) + '&SITE_ID=<?=$SITE_ID?>');
		}
	},
