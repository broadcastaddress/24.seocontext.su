...
<?
	...
	foreach($arResult['DEAL'] as $sKey =>  $arDeal)
	{
		...
		if ($arDeal['EDIT'] && $arResult['IS_BIZPROC_AVAILABLE']) :
			$arActions[] = array('SEPARATOR' => true);
...
{
	// Setup STAGE_ID -->
	$stages = '<div id="ACTION_STAGE_WRAPPER" style="display:none;">';
	$stages .= '<select name="ACTION_STAGE_ID" size="1">';
	...
	{
		foreach($arResult['CATEGORY_STAGE_LIST'] as $id => $name)
		{
	...
	elseif(isset($arResult['CATEGORY_STAGE_GROUPS']))
	{
		foreach($arResult['CATEGORY_STAGE_GROUPS'] as $group)
		{
			...
			if($groupName !== '')
			{
				$stages .= '<optgroup label="'.htmlspecialcharsbx($groupName).'">';
			}

			$groupItems = isset($group['items']) && is_array($group['items']) ? $group['items'] : array();
			foreach($groupItems as $itemKey => $itemName)
			{
				$stages .= '<option value="'.htmlspecialcharsbx($itemKey).'">'.htmlspecialcharsbx($itemName).'</option>';
			}
		...
		<script type="text/javascript">
			BX.ready(
				...
				BX.bind(
					...
					"change",
					function(e){
						var stageWrapper = BX("ACTION_STAGE_WRAPPER");
						if(stageWrapper)
						{
							stageWrapper.style.display = select.value === "set_stage" ? "" : "none";
						}
...
if($isEditable)
{
	...
	if($arResult['PERMS']['WRITE'])
	{
		...
		$arActionList['refresh_account'] = GetMessage('CRM_DEAL_REFRESH_ACCOUNT');
