...
if (!$bInternal)
{
	...
	$GLOBALS['APPLICATION']->IncludeComponent('bitrix:crm.entity.selector',
		'',
		array(
			'ENTITY_TYPE' => Array('COMPANY'),
			'INPUT_NAME' => 'UF_MYCOMPANY_ID',
			'INPUT_VALUE' =>  isset($_REQUEST["UF_MYCOMPANY_ID"]) ? $_REQUEST["UF_MYCOMPANY_ID"] : '',
			'FORM_NAME' => $arResult['GRID_ID'],
			'MULTIPLE' => 'N',
			'FILTER' => true
		),
	...
	$myCompanyFilterHtml = ob_get_contents();
	ob_end_clean();
	...
	$arResult['FILTER'] = array(
		...
		array('id' => 'UF_MYCOMPANY_ID', 'name' => GetMessage('CRM_COLUMN_UF_MYCOMPANY_ID'), 'default' => false, 'type' => 'custom', 'value' => $myCompanyFilterHtml),
	);
...
$arResult['HEADERS'] = array(
	...
	array('id' => 'UF_MYCOMPANY_ID', 'name' => GetMessage('CRM_COLUMN_UF_MYCOMPANY_ID'), 'default' => true, 'editable' => false),
...
foreach ($arFilter as $k => $v)
{
	...
	elseif ($k === 'UF_MYCOMPANY_ID')
	{
		unset($arFilter[$k]);
		$ownerData =explode('_', $v);
		if (count($ownerData) === 1)
		{
			$ownerID = intval($ownerData[0]);
			if($ownerID > 0)
			{
				$arFilter['UF_MYCOMPANY_ID'] = $ownerID;
			}
		}
		if(count($ownerData) > 1)
		{
			$ownerTypeName = CCrmOwnerType::ResolveName(CCrmOwnerType::ResolveID($ownerData[0]));
			$ownerID = intval($ownerData[1]);
			if($ownerTypeName === 'COMPANY' && $ownerID > 0)
			{
				$arFilter['UF_MYCOMPANY_ID'] = $ownerID;
			}
		}
	...
	if ($dbRes)
	{
		...
		while ($arContact = $dbRes->Fetch())
		{
				...
				&& count($arContactList[$arContact['ID']]) > 0)
			{
				...
				{
					$arResult['INVOICE'][$invoiceId]['CONTACT_FORMATTED_NAME'] = $contactFormattedName = CCrmContact::PrepareFormattedName(
						array(
