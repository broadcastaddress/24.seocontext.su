...
class CAllCrmCatalog
{
	...
	public static function GetDefaultID()
	{
		...
		{
			AddMessage2Log(
				'-- [george]: 1 - \CCrmCatalog::GetDefaultID --'.PHP_EOL.
				'$ID = intval(COption::GetOptionString(\'crm\', \'default_product_catalog_id\', \'0\'));'.PHP_EOL.
				'$ID = '.var_export($ID, true).';'.PHP_EOL,
				'crm'
			);
		}
		...
		// debug
		$iblockId = CIBlock::GetArrayByID($ID, 'ID');
		if($ID > 0 && intval($iblockId) !== $ID)
		{
			AddMessage2Log(
				'-- [george]: 2 - \CCrmCatalog::GetDefaultID --'.PHP_EOL.
				'$iblockId = CIBlock::GetArrayByID($ID, \'ID\');'.PHP_EOL.
				'if($ID > 0 && intval($iblockId) !== $ID)'.PHP_EOL.
				'$ID = '.var_export($ID, true).';'.PHP_EOL.
				'$iblockId = '.var_export($iblockId, true).';'.PHP_EOL.
				'...'.PHP_EOL.
				'$ID = 0;'.PHP_EOL,
				'crm'
			);
	...
	public static function EnsureDefaultExists()
	{
		...
		if($ID <= 0)
		{
			if(($ID = self::CreateCatalog()) > 0)
			{
				// debug
				AddMessage2Log(
					'-- [george]: 7 - \CCrmCatalog::EnsureDefaultExists --'.PHP_EOL.
					'if(($ID = self::CreateCatalog()) > 0)'.PHP_EOL.
					'COption::SetOptionString(\'crm\', \'default_product_catalog_id\', '.var_export($ID, true).');'.PHP_EOL.
					'return '.var_export($ID, true).';'.PHP_EOL,
					'crm'
				);
				...
				// debug
				AddMessage2Log(
					'-- [george]: 7 - 1 - \CCrmCatalog::EnsureDefaultExists --'.PHP_EOL.
					'After: COption::SetOptionString(\'crm\', \'default_product_catalog_id\', '.var_export($ID, true).');'.PHP_EOL.
					'return '.var_export($ID, true).';'.PHP_EOL,
					'crm'
				);
			}
			else    // debug
			{
				// debug
				AddMessage2Log(
					'-- [george]: 8 - \CCrmCatalog::EnsureDefaultExists --'.PHP_EOL.
					'else    // if(($ID = self::CreateCatalog()) > 0)'.PHP_EOL.
					'return '.var_export($ID, true).';'.PHP_EOL,
					'crm'
				);
			}
	...
	public static function CreateCatalog($originatorID = '', $name = '', $siteID = null)
	{
		...
		if (!$rsIBlockTypes->Fetch())
		{
			...
			if(!$result)
			{
				...
				// debug
				AddMessage2Log(
					'-- [george]: 3 - \CCrmCatalog::CreateCatalog --'.PHP_EOL.
					'$result = $iblocktype->Add('.PHP_EOL.
					'$iblocktype->LAST_ERROR = '.var_export($iblocktype->LAST_ERROR, true).';'.PHP_EOL.
		...
		if($iblockID === false)
		{
			...
			// debug
			AddMessage2Log(
				'-- [george]: 4 - \CCrmCatalog::CreateCatalog --'.PHP_EOL.
				'$iblockID = $iblock->Add('.PHP_EOL.
				'$iblock->LAST_ERROR = '.var_export($iblock->LAST_ERROR, true).';'.PHP_EOL.
				'return false;'.PHP_EOL,
				'crm'
			);
		...
		if($result === false)
		{
			...
			// debug
			AddMessage2Log(
				'-- [george]: 5 - \CCrmCatalog::CreateCatalog --'.PHP_EOL.
				'$result = CCrmCatalog::Add(array('.var_export($iblockID, true).','.var_export($originatorID, true).'));'.PHP_EOL.
				'return false;'.PHP_EOL,
				'crm'
			);
		...
		// debug
		AddMessage2Log(
			'-- [george]: 6 - \CCrmCatalog::CreateCatalog --'.PHP_EOL.
			'return '.var_export($iblockID, true).';'.PHP_EOL,
			'crm'
		);
