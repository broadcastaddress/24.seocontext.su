	...
	static public function BuildSql($permEntity, $sAliasPrefix, $mPermType, $arOptions = array())
	{
		...
		foreach($permissionSets as &$permissionSet)
		{
			...
			if($scopeQty === 0)
			{
				...
				if($restrictionSql !== '')
				{
					$subQueries[] = "SELECT {$sAliasPrefix}P.ENTITY_ID FROM b_crm_entity_perms {$sAliasPrefix}P WHERE {$sAliasPrefix}P.ENTITY = '{$permEntity}' AND {$restrictionSql}";
			...
			else
			{
				...
				if($restrictionSql !== '')
				{
					$subQueries[] = "SELECT {$sAliasPrefix}P2.ENTITY_ID FROM b_crm_entity_perms {$sAliasPrefix}P1 INNER JOIN b_crm_entity_perms {$sAliasPrefix}P2 ON {$sAliasPrefix}P1.ENTITY = '{$permEntity}' AND {$sAliasPrefix}P2.ENTITY = '{$permEntity}' AND {$sAliasPrefix}P1.ENTITY_ID = {$sAliasPrefix}P2.ENTITY_ID AND {$restrictionSql} AND {$scopeSql}";
				}
				else
				{
					$subQueries[] = "SELECT {$sAliasPrefix}P2.ENTITY_ID FROM b_crm_entity_perms {$sAliasPrefix}P2 WHERE {$sAliasPrefix}P2.ENTITY = '{$permEntity}' AND {$scopeSql}";
				}
		...
		if(isset($arOptions['READ_ALL']) && $arOptions['READ_ALL'] === true)
		{
			...
			$subQueries[] = "SELECT {$sAliasPrefix}P.ENTITY_ID FROM b_crm_entity_perms {$sAliasPrefix}P WHERE {$sAliasPrefix}P.ENTITY = '{$permEntity}' AND {$sAliasPrefix}P.ATTR = '{$readAll}'";
		}
