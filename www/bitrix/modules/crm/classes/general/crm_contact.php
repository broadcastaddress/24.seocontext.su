...
class CAllCrmContact
{
	...
	public static function __AfterPrepareSql($sender, $arOrder, $arFilter, $arGroupBy, $arSelectFields)
	{
		...
		{
			$sqlData[] = ContactCompanyTable::prepareFilterJoinSql(
		...
		{
			$sqlData[] = Crm\Binding\DealContactTable::prepareFilterJoinSql(
				CCrmOwnerType::Deal,
		...
		if(isset($arFilter['ADDRESSES']))
		{
			...
			{
				$sqlData[] = EntityAddress::prepareFilterJoinSql(
		...
		return !empty($sqlData) ? array('FROM' => implode(' ', $sqlData)) : false;
	}
	...
	public static function PrepareFilter(&$arFilter, $arFilter2Logic = null)
	{
		...
		static $arImmutableFilters = array(
			...
			'TYPE_ID', 'SOURCE_ID',
			'HAS_PHONE', 'HAS_EMAIL',
			'GRID_FILTER_APPLIED', 'GRID_FILTER_ID'
		);
