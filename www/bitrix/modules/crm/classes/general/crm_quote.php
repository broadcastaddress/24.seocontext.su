...
class CAllCrmQuote
{
	...
	public function Add(&$arFields, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			{
				\Bitrix\Crm\Binding\QuoteContactTable::bindContacts($ID, $contactBindings);
	...
	public function Update($ID, &$arFields, $bCompare = true, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			//region Preparation of contacts
			$originalContactBindings = \Bitrix\Crm\Binding\QuoteContactTable::getQuoteBindings($ID);
			...
			*/
			...
			if(is_array($contactBindings))
			{
				...
				\Bitrix\Crm\Binding\EntityBinding::prepareBindingChanges(
					CCrmOwnerType::Contact,
					\Bitrix\Crm\Binding\QuoteContactTable::getQuoteBindings($ID),
					$contactBindings,
			...
			{
				\Bitrix\Crm\Binding\QuoteContactTable::unbindContacts($ID, $removedContactBindings);
			}

			if(!empty($addedContactBindings))
			{
				\Bitrix\Crm\Binding\QuoteContactTable::bindContacts($ID, $addedContactBindings);
			}
			//endregion
	...
	public function Delete($ID, $options = array())
	{
		...
		if (is_object($dbRes) && $dbRes->AffectedRowsCount() > 0)
		{
			...
			\Bitrix\Crm\Binding\QuoteContactTable::unbindAllContacts($ID);
	...
	public static function SaveProductRows($ID, $arRows, $checkPerms = true, $regEvent = true, $syncOwner = true)
	{
		...
		$result = CCrmProductRow::SaveRows(self::OWNER_TYPE, $ID, $arRows, $context, $checkPerms, $regEvent, $syncOwner);
		if($result)
		{
	...
	public static function PrepareSalePaymentData(array &$arQuote)
	{
		...
		if (CModule::IncludeModule('iblock'))
		{
			if ($calculatedOrder['BASKET_ITEMS'])
			{
				...
				{
					$product = \CCrmProduct::GetByID($row['PRODUCT_ID'], true);
					...
					while ($arProperty = $rsProperties->Fetch())
						$calculatedOrder['BASKET_ITEMS'][$i]['PROPERTY_'.$arProperty['ID']] = $arProperty['VALUE'];
				}
			}
		}
