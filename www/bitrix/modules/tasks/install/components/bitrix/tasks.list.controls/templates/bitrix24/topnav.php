...
)
{
	...
	<div class="task-main-wrap" id="task-main-wrap">
			...
			<div class="tasks-top-menu-wrap">
				<div class="tasks-top-menu" id="task-menu-block">
					...
					if(is_array($arResult['VIEW_STATE']['SPECIAL_PRESETS']))
					{
						...
						{
							?><span class="tasks-top-item-wrap <?php if ($arResult['MARK_SPECIAL_PRESET'] === 'Y' && $preset['SELECTED'] === 'Y') echo ' tasks-top-item-wrap-active'; ?> ">
					...
					?><span class="tasks-top-item-wrap <?php if ($arParams['MARK_SECTION_REPORTS'] === 'Y') echo ' tasks-top-item-wrap-active'; ?>">
						...
						<a class="tasks-top-item" href="/marketplace/?category=tasks" target="_blank">
			...
			<script type="text/javascript">
				...
				more_btn_toggle.init({
					...
					button:BX.create('span', {
						...
						html:'<a href="javascript:void(0)" class="tasks-top-item-more">\
					...
					popup_items:[
						...
						,{
							text : '<?php echo GetMessageJs('TASKS_PANEL_TAB_APPLICATIONS'); ?>', 
		...
		{
			// todo: highly recommended to refactor the following

			$spanText = '<span class="task-top-panel-text">';
			$spanEnd = '</span>';
			...
			if ($arResult['F_CREATED_BY'])
			{
				if ($arResult['F_CREATED_BY'] == $USER->getId())
					$originatorName = GetMessage('TASKS_PANEL_HUMAN_FILTER_STRING_RESPONSIBLE_IS_ME');
				else
					$originatorName = htmlspecialcharsbx($arResult['~USER_NAMES'][$arResult['F_CREATED_BY']]);
			}
			else
				$originatorName = GetMessage('TASKS_PANEL_HUMAN_FILTER_STRING_ANY_ORIGINATOR');
			...
			ob_start();
			?>
				<span id="task-top-panel-task-category-selector" class="webform-small-button webform-small-button-transparent bx-filter-button">
					...
					?></span><span class="webform-small-button-icon">
				</span>
				</span>
			<?
			$categorySelector = ob_get_clean();

			ob_start();
			?>

			<span id="task-top-panel-task-originator-selector" class="webform-small-button webform-small-button-transparent bx-filter-button">
			...
			<?php
			$originatorButtonHTML = ob_get_clean();

			ob_start();
			?>

				<span id="task-top-panel-task-responsible-selector" class="webform-small-button webform-small-button-transparent bx-filter-button">
					<span class="webform-small-button-text">
						<?=$responsibleName?>
					</span>
			...
			<?
			$responsibleButtonHTML = ob_get_clean();

			ob_start();
			?>

				<span class="task-top-panel-text task-top-panel-switch" id="task-top-panel-from-for-switch">
					<a href="javascript:void(0)">
						<span data-bx-ui-id="from-for-switch-label" data-label="FROM">
							<?=GetMessage('TASKS_PANEL_HUMAN_FILTER_STRING_FROM')?>
						</span>
						<span data-bx-ui-id="from-for-switch-label" data-label="FOR">
							<?=GetMessage('TASKS_PANEL_HUMAN_FILTER_STRING_FOR')?>
						</span>
					</a>
				</span>

			<?
			$personTypeButtonHTML = ob_get_clean();
			?>
			...
			<div class="task-top-panel-two">
				...
				$role = '';
				if(LANGUAGE_ID != 'de')
					$role = strtolower($arResult['VIEW_STATE']['ROLES'][$selectedRoleCodename]['TITLE_ALT']);
				else
					$role = $arResult['VIEW_STATE']['ROLES'][$selectedRoleCodename]['TITLE_ALT'];

				if ($arResult['SELECTED_SECTION_NAME'] === 'VIEW_SECTION_ROLES') // role filter (4 buttons)
				{
					if ($selectedRoleCodename === 'VIEW_ROLE_RESPONSIBLE')
					{
						echo str_replace(
							array('#ROLE#', '#STATUS#', '#ORIGINATOR#', '#SPAN_TEXT#', '#SPAN_END#'),
							array($role, $categorySelector, $originatorButtonHTML, $spanText, $spanEnd),
					...
					{
						echo str_replace(
							array('#ROLE#', '#STATUS#', '#RESPONSIBLE#', '#SPAN_TEXT#', '#SPAN_END#'),
							array($role, $categorySelector, $responsibleButtonHTML, $spanText, $spanEnd),
					...
					{
						echo str_replace(
							array('#ROLE#', '#STATUS#', '#SPAN_TEXT#', '#SPAN_END#', '#TYPE#', '#PERSON#'),
							array(
								$role,
								$categorySelector,
								$spanText,
								$spanEnd,
								$personTypeButtonHTML,
								$responsibleButtonHTML.$originatorButtonHTML
							),
							GetMessage('TASKS_PANEL_HUMAN_FILTER_STRING_MATRIX_FOR_AUDITOR_ACCOMPLICE')
						);
					}
				}
				elseif ($arResult['MARK_SPECIAL_PRESET'] === 'Y') // special presets
				{
					echo str_replace(
						array('#STATUS#', '#SPAN_TEXT#', '#SPAN_END#', '#TYPE#', '#PERSON#'),
						array(
							$categorySelector,
							$spanText,
							$spanEnd,
						...
						GetMessage('TASKS_PANEL_HUMAN_FILTER_STRING_MATRIX_FOR_SPECIAL_PRESET')
					);
				}
				elseif ($arResult['MARK_SECTION_ALL'] === 'Y') // "all" + advanced filter
				{
				...
				<div class="task-top-panel-two-inright">
					<span class="task-top-panel-text"><?=GetMessage('TASKS_PANEL_TAB_VIEW_SELECTOR_TITLE')?></span>
					...
					if(typeof sw != 'undefined' && sw != null)
					{
						...
						if(buttons != null)
						{
							...
							for(var k = 0; k < buttons.length; k++)
							{
								...
								BX[label == way ? 'show' : 'hide'](buttons[k]);
							}

							// switch buttons
							if(way == 'FOR')
							{
								BX.hide(BX('task-top-panel-task-originator-selector'));
								BX.show(BX('task-top-panel-task-responsible-selector'));
							}
							else
							{
								BX.show(BX('task-top-panel-task-originator-selector'));
								BX.hide(BX('task-top-panel-task-responsible-selector'));
							}
