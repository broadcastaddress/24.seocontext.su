	...
	public static function onSearchIndex($userField)
	{
		...
		foreach($values as $value)
		{
			...
			if($type == self::TYPE_NEW_OBJECT)
			{
				...
				{
					$searchData[] = $contentManager->getFileContent(self::getFileById($realValue));
				}
				else
				{
		...
		if($attachedIdsForLoad)
		{
			...
			{
				$searchData[] = $contentManager->getObjectContent($attachedObject->getObject());
			}
		}
		...
		if($fileIdsForLoad)
		{
			...
			{
				$searchData[] = $contentManager->getObjectContent($file);
			}
		}
