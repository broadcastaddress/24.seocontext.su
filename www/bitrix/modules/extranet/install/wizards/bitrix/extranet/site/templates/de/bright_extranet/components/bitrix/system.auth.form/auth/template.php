...
<div onclick="return authFormWindow.CloseLoginForm()" id="close-form-window" title="Close window">Schließen</div><b>Anmeldung</b>
</div>
...
function ShowAuthMenu(el)
{
	...
	<div class="auth-links">
		...
		if(CModule::IncludeModule("bizproc") && IsModuleInstalled("socialnetwork"))
		{
			...
			{
				echo '<span class="auth-icon-active"><a href="'.SITE_DIR.'contacts/personal/bizproc/">'.GetMessage('AUTH_BZP').'</a></span>';
			}
			else
				echo '<span class="auth-icon"><a href="'.SITE_DIR.'contacts/personal/bizproc/">'.GetMessage('AUTH_BZP').'</a></span>';
		}
		?>
		<?if ($isNTLM === false):?><span>|</span>&nbsp;<a href="<?=$logoutUrl?>"><?=GetMessage("AUTH_LOGOUT_BUTTON")?></a><?endif?>
	</div>
<?endif?>
