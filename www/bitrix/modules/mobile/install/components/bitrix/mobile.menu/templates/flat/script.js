...
//		app.exec("setButtonBadge", {code: "menu", value: totalCount});
	...
	BX.Menu.prototype.userList = function ()
	{
		...
		{
			if(BX.localStorage.get("carddav_ask") != 6)
			{
				BX.localStorage.set("carddav_ask", 6);
				app.confirm({
					text:this.MenuSettings.lang.ask_sync_carddav,
					buttons:[this.MenuSettings.lang.ask_sync_yes,this.MenuSettings.lang.ask_sync_no],
					callback:function(buttonIndex){
						if(buttonIndex == 1)
						{
							app.exec("createSyncAccount", {type:"carddav"});
						}
					}
