var Bitrix24FormLoader = {
	...
	load: function(params)
	{
		...
		if(params.handlers.init)
		{
			params.handlers.init.apply(this, [params]);
		}
		...
		switch (data.action)
		{
			...
			case 'keyboard':
				if (data.value == 27)
				{
					...
					if(form.handlers.keyboard)
					{
						form.handlers.keyboard.apply(this, [form, data.value]);
					}
				}
				break;
			case 'event':
				var form = this.forms[data.uniqueLoadId || uniqueLoadId];
				if(form && form.handlers[data.eventName])
				{
					form.handlers[data.eventName].apply(this, data.value);
				}
				break;
			case 'analytics':
	...
	onFrameLoad: function(uniqueLoadId)
	{
		...
		if(form.handlers.load)
		{
			form.handlers.load.apply(this, [form]);
		}
	},
