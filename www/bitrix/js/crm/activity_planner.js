...
(function(BX)
{
	...
	Planner.prototype.showEdit = function (params)
	{
		...
		this._createAjaxPopup(params, function(editorNode)
		{
			...
			me.communications = new Communications(
				me.getNode('communications-container'),
				{
	...
	Planner.prototype.checkPlannerState = function (params)
	{
		...
		{
			updateParams.from = BX.formatDate(new Date((fromDate.getTime() - dayLength * 3)), BX.message('FORMAT_DATE'));
			updateParams.to = BX.formatDate(new Date((fromDate.getTime() + dayLength * 10)), BX.message('FORMAT_DATE'));
	...
	Destination.prototype.addItem = function(item, type)
	{
		...
		if (!BX.findChild(items, { attr : { 'data-id' : item.id }}, false, false))
		{
			...
			containerText.innerHTML = BX.util.htmlspecialchars(item.name);
	...
	Communications.prototype.addItem = function(item, closeDialog)
	{
		...
		containerText.innerHTML = [
			BX.util.htmlspecialchars(item.entityTitle),
			BX.util.htmlspecialchars(item.value)
		].join(' ');
		...
		BX.CrmActivityProvider.prototype =
		{
			...
			_prepareViewDlgButtons: function ()
			{
				...
				else
				{
					...
					if(this.getOption('enableEditButton', true) && (this.getType() ===  BX.CrmActivityType.call || this.getType() === BX.CrmActivityType.meeting))
					{
						result.push(
							{
								type: 'link',
