...
if ($productID > 0/* && count($arProps) > 0*/)
{
	...
	while ($arProperty = $rsProperties->Fetch())
	{
		...
		{
			$arProp = $arProps[$propID];
			$arProp['PROPERTY_USER_TYPE'] = $arPropUserTypeList[$arProperty['USER_TYPE']];
			$arPropertyValues[$propID][] = call_user_func_array($arPropUserTypeList[$arProperty['USER_TYPE']]["GetPublicViewHTML"], array(
				$arProp,
