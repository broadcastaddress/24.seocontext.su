...
if (!$bInternal)
{
	...
	$GLOBALS['APPLICATION']->IncludeComponent('bitrix:crm.entity.selector',
		...
		array(
			'ENTITY_TYPE' => 'COMPANY',
			'INPUT_NAME' => 'MYCOMPANY_ID',
			'INPUT_VALUE' => isset($_REQUEST['MYCOMPANY_ID']) ? intval($_REQUEST['MYCOMPANY_ID']) : '',
			'FORM_NAME' => $arResult['GRID_ID'],
			'MULTIPLE' => 'N',
			'FILTER' => true
		),
	...
	$arResult['FILTER'] = array(
		...
		array('id' => 'MYCOMPANY_ID', 'name' => GetMessage('CRM_COLUMN_MYCOMPANY_LIST'), 'type' => 'custom', 'value' => $sValMyCompany),
...
$arResult['HEADERS'] = array_merge(
	...
	array(
		...
		array('id' => 'MYCOMPANY_ID', 'name' => GetMessage('CRM_COLUMN_MYCOMPANY_ID'), 'sort' => 'mycompany_id', 'default' => false, 'editable' => false),
...
$arImmutableFilters = array(
	'FM', 'ID', 'ASSIGNED_BY_ID', 'CURRENCY_ID', 'CONTACT_ID', 'COMPANY_ID', 'LEAD_ID', 'DEAL_ID', 'MYCOMPANY_ID',
	'CREATED_BY_ID', 'MODIFY_BY_ID', 'PRODUCT_ROW_PRODUCT_ID', 'WEBFORM_ID'
);
...
$arResult['PAGINATION'] = array(
	...
	'URL' => $APPLICATION->GetCurPageParam('', array('apply_filter', 'clear_filter', 'save'))
);
