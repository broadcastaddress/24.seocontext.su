var CrmFormEditor = function(params)
{
	...
	this.init = function (params)
	{
		...
		this.responsible = new CrmFormEditorResponsible({
			caller: this,
			context: BX('CRM_WEBFORM_RESPONSIBLE')
		});
		/* init filter of field selector */
...
function CrmFormEditorResponsible(params)
{
	this.caller = params.caller;
	this.context = params.context;

	this.attributeContainer = 'data-bx-webform-responsible-cont';
	this.container = this.context.querySelector(this.attributeContainer);

	this.helper = BX.CrmFormEditorHelper;
	this.init(params);
}
CrmFormEditorResponsible.prototype =
{
	init: function ()
	{
		this.initControl();
	...
	initControl: function ()
	{
		var destinationParams = {
			...
			'searchInput' : null, //
			'extranetUser' :  false,
			'bindMainPopup' : { node : this.container, 'offsetTop' : '5px', 'offsetLeft': '15px'}, //
			'bindSearchPopup' : { node : this.container, 'offsetTop' : '5px', 'offsetLeft': '15px'}, //
		...
		BX.SocNetLogDestination.init(destinationParams);
	}
};
...
CrmFormEditorFieldPreset.prototype =
{
	...
	actualize: function (field)
	{
		...
		if(isList && fieldData.items && fieldData.items.length > 0)
		{
			var fieldDataItems = fieldData.items;
