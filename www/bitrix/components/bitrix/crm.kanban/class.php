...
class CrmKanbanComponent extends \CBitrixComponent
{
	...
	protected $selectPresets = array(
								...
								'invoice' => array('ID', 'STATUS_ID', 'DATE_INSERT', 'DATE_INSERT_FORMAT', 'DATE_BILL', 'ORDER_TOPIC', 'PRICE', 'CURRENCY', 'UF_CONTACT_ID', 'UF_COMPANY_ID'),
						);
	...
	protected function init()
	{
		...
		{
			$this->arParams['EXTRA']['counter'] = htmlspecialcharsbx($request['counter']);
		}
	...
	protected function getFilter()
	{
		...
		if ($filter === null)
		{
			...
			if (!isset($search['GRID_FILTER_APPLIED']))
			{
				$search = array();
			}
			if (!empty($search))
			{
				foreach ($gridFilter as $key => $item)
				{
					...
					{
						if (isset($search[$key . '_from']))
						{
							$filter['>='.$key] = $search[$key . '_from'];
						}
						if (isset($search[$key . '_to']))
						{
			...
			/*if ($this->type != $this->types['invoice'] && !isset($filter['ASSIGNED_BY_ID']))
			{
				$filter['ASSIGNED_BY_ID'] = $this->uid;
			}*/
			//counters
			if ($this->type == $this->types['lead'] || $this->type == $this->types['deal'])
			{
				if (isset($this->arParams['EXTRA']['counter']))
				{
					$filter = array();
					$filterCounter = null;
					$counterTypeID = \Bitrix\Crm\Counter\EntityCounterType::resolveID($this->arParams['EXTRA']['counter']);
					if (\Bitrix\Crm\Counter\EntityCounterType::isDefined($counterTypeID))
					{
						try
						{
							$counter = \Bitrix\Crm\Counter\EntityCounterFactory::create(
								...
								$counterTypeID,
								$this->uid,
								\Bitrix\Crm\Counter\EntityCounter::internalizeExtras(array('counter' => $this->arParams['EXTRA']['counter']))
							);
	...
	protected function fillFMfields(array $items, $contragent)
	{
		...
		if (!empty($items))
		{
			...
			while ($row = $res->fetch())
			{
				...
				if ($row['TYPE_ID'] == 'im' && strpos($row['VALUE'], 'imol|') !== 0)
				{
					continue;
				}
	...
	protected function getColumns($clear = false)
	{
		...
		if (empty($columns))
		{
			...
			else
			{
				...
				{
					//@todo filter can be changed
					if ($this->type == $types['deal'])
					{
						if (isset($baseFilter['CATEGORY_ID']))
						{
							$filter['CATEGORY_ID'] = $baseFilter['CATEGORY_ID'];
						}
						$statusCode = 'STAGE_ID';
					}
					else
			...
			if ($stats)
			{
				$statusCode = $this->type == $types['deal'] ? 'STAGE_ID' : 'STATUS_ID';
	...
	protected function getItems(array $filter = array(), $pagen = true)
	{
		...
		$statusKey = $type == $types['deal'] ? 'STAGE_ID' : 'STATUS_ID';
		...
		if (class_exists($provider))
		{
			...
			while ($row = $res->fetch())
			{
				...
				if ($type == $types['lead'])
				{
					...
					$row['DATE'] = \ConvertDateTime($row['DATE_CREATE'], str_replace(':SS', '', FORMAT_DATETIME));
				}
				elseif ($type == $types['deal'])
				{
					$row['PRICE'] = $row['OPPORTUNITY'];
					$row['DATE'] = $row['BEGINDATE'] ? $row['BEGINDATE'] : $row['DATE_CREATE'];
				}
				elseif ($type == $types['quote'])
				{
					$row['PRICE'] = $row['OPPORTUNITY'];
					$row['DATE'] = \ConvertDateTime($row['BEGINDATE'] ? $row['BEGINDATE'] : $row['DATE_CREATE'], FORMAT_DATE);
				}
				...
				$result[$row['ID']] = array(
					...
					'columnId' => $columnId = htmlspecialcharsbx(array_key_exists('STAGE_ID', $row) ? $row['STAGE_ID'] : $row['STATUS_ID']),
					'columnColor' => isset($columns[$columnId]) ? $columns[$columnId]['color'] : '',
					'price' => $row['PRICE'],
					'price_formatted' => $row['PRICE_FORMATTED'],
					'date' => htmlspecialcharsbx($row['DATE']),
	...
	protected function makeAction()
	{
		...
		if (($action = $request->get('action')) && ($id = $request->get('entity_id')) && check_bitrix_sessid())
		{
			...
			if ($action == 'status' && ($status = $request->get('status')) && isset($statuses[$status]))
			{
				...
				else
				{
					...
					{
						$finalError = false;
						//if lead, check not converted status
						if ($type == $types['lead'])
						{
							if ($row[$statusKey] != $status && $statuses[$row[$statusKey]]['PROGRESS_TYPE'] != 'PROGRESS')
							{
								$finalError = true;
						...
						//update
						if ($row[$statusKey] != $status && !$finalError)
						{
		...
		if ($request->get('apply_filter') == 'Y' || isset($this->arParams['EXTRA']['counter']))
		{
	...
	public function executeComponent()
	{
		...
		else
		{
			...
			if (!empty($this->arParams['ADDITIONAL_FILTER']))
			{
				...
				if (isset($filter['COLUMN']))
				{
					...
					if ($type == $types['deal'])
					{
						$filter['STAGE_ID'] = $filter['COLUMN'];
					}
					else
					{
						$filter['STATUS_ID'] = $filter['COLUMN'];
					}
					unset($filter['COLUMN']);
				}
			...
			else
			{
				foreach ($columns as $k => $column)
				{
					if ($column['type'] != 'LOOSE')
					{
						...
						if ($type == $types['deal'])
						{
							$filter['STAGE_ID'] = $column['id'];
						}
						else
						{
							$filter['STATUS_ID'] = $column['id'];
						}
						if ($column['type'] == 'WIN')
						{
							$filter[$this->winPeriodKey] = $winPeriod;
		...
		{
			$GLOBALS['APPLICATION']->setTitle(Loc::getMessage('CRM_KANBAN_TITLE_' . $this->type));
