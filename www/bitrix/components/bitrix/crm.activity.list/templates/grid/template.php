...
foreach($arResult['ITEMS'] as &$item)
{
	...
	if($isEditable)
	{
		if($item['CAN_EDIT'] && ($itemTypeID === CCrmActivityType::Call || $itemTypeID === CCrmActivityType::Meeting || $itemTypeID === CCrmActivityType::Provider))
		{
			$arActions[] = array(
				'ICONCLASS' => 'edit',
	...
	elseif($typeID === CCrmActivityType::Provider && ($provider = CCrmActivity::GetActivityProvider($item)) !== null):
...
if($isEditable)
{
	...
	CCrmViewHelper::RenderUserSearch(
		"{$prefix}_ACTION_RESPONSIBLE",
		"ACTION_RESPONSIBLE_SEARCH",
	...
	$actionHtml .= '<div id="ACTION_RESPONSIBLE_WRAPPER" style="display:none;">'.ob_get_clean().'</div>';
	$actionHtml .= '
		<script type="text/javascript">
			BX.ready(
				function()
				{
					var select = BX.findChild(
						BX.findPreviousSibling(
							BX.findParent(
								BX("ACTION_RESPONSIBLE_WRAPPER"),
								{ "tagName": "td" }
							)
						),
						{ "tagName": "select" }
					...
					BX.bind(
						...
						function(e)
						{
							BX("ACTION_RESPONSIBLE_WRAPPER").style.display = select.value === "assign_to" ? "" : "none";
						}
					)
				}
		);
	...
	$arActionList['assign_to'] = GetMessage('CRM_ACTION_ASSIGN_TO');
}
	...
	array(
		...
		'FOOTER' => array(
			array(
				...
				'service_url' => '/bitrix/components/bitrix/crm.activity.list/list.ajax.php?'.bitrix_sessid_get()
			),
		...
		'FILTER_TEMPLATE' => $enableToolbar && $useQuickFilter ? 'popup' : 'flat',
