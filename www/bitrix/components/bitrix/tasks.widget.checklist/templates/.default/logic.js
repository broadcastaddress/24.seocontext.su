...
(function(){
			...
			toggleItemMode: function(apply, item)
			{
				if(item.isEditMode())
				{
					if(apply)
					{
						...
						item.lockInput();
						this.getSyncer().perform([['update', {id: item.value(), data:{TITLE: title}}]]).then(function(){
	...
	BX.Tasks.Component.TasksWidgetCheckList.ItemManager.Item = BX.Tasks.Util.ItemSet.Item.extend({
		...
		methods: {
			...
			title: function(title)
			{
				if(typeof title != 'undefined')
				{
					...
					this.data().TITLE_HTML = BX.util.htmlspecialchars(title); // todo: need to have basic client-side BBCODE->HTML converter here
					this.data().DISPLAY = this.data().TITLE_HTML;

					// update dom here... where is react?
					this.control('title').innerHTML = this.data().DISPLAY;
		...
		methodsStatic: {
			...
			{
				data.DISPLAY = data.TITLE; // todo: need to have basic client-side BBCODE->HTML converter here
