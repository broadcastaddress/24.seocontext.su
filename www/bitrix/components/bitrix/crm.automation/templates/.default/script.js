...
BX.Crm.Automation = (function(BX)
{
	...
	Template.prototype =
	{
		...
		onAddButtonClick: function(button)
		{
			...
			var title, availableRobots = this.manager.getAvailableRobots();
			for (i = 0; i < availableRobots.length; ++i)
			{
				if (availableRobots[i]['EXCLUDED'])
					continue;
				title = availableRobots[i].NAME;
				if (availableRobots[i]['ROBOT_SETTINGS'] && availableRobots[i]['ROBOT_SETTINGS']['TITLE'])
					title = availableRobots[i]['ROBOT_SETTINGS']['TITLE'];

				//TODO: remove after aprooving app
				var isDemo = availableRobots[i]['ROBOT_SETTINGS'] && availableRobots[i]['ROBOT_SETTINGS']['IS_DEMO'];
				...
				menuItems.push({
					...
					isDemo: isDemo,
					documentStatus: button.getAttribute('data-status-id'),
					...
					onclick: function(e, item)
					{
						if (!item.isDemo)
						{
							...
							me.addRobot(item.robotData, function(robot)
							{
		...
		initRobotSettingsControls: function(robot, form)
		{
			this.robotSettingsControls = [];

			var i, userSelectors = form.querySelectorAll('[data-role="user-selector"]');
			...
			var inlineSelectors = form.querySelectorAll('[data-role="inline-selector-target"]');
			...
			var timeSelectors = form.querySelectorAll('[data-role="time-selector"]');
		...
		renderDelaySettings: function(robot)
		{
			...
			var executeAfterPreviousInput1 = BX.create("input", {
				attrs: {
			...
			var executeAfterPreviousInput2 = BX.create("input", {
				attrs: {
					className: "crm-automation-popup-select-input",
					type: "radio",
					id: "param-group-3-2" + idSalt,
					name: "execute_after_previous",
					value: '1'
				}
			});
			...
			if (isExecuteAfterPrevious)
				executeAfterPreviousInput2.setAttribute('checked', 'checked');
			else
				executeAfterPreviousInput1.setAttribute('checked', 'checked');
				...
				children: [
					BX.create("div", {
						...
						children: [
							...
							executeAfterPreviousInput1,
							BX.create("label", {
								attrs: {
									className: "crm-automation-popup-settings-link",
									for: "param-group-3-1" + idSalt
								},
								text: BX.message('CRM_AUTOMATION_CMP_AT_ONCE')
							}),
							executeAfterPreviousInput2,
							BX.create("label", {
								attrs: {
									className: "crm-automation-popup-settings-link",
									for: "param-group-3-2" + idSalt
								},
								text: BX.message('CRM_AUTOMATION_CMP_AFTER_PREVIOUS')
							})
						]
					}),
					BX.create("div", {
						...
						children: [
							...
							BX.create("span", {
								attrs: { className: "crm-automation-popup-settings-title" },
								text: BX.message('CRM_AUTOMATION_CMP_CHOOSE_TIME') + ":"
							}),
		...
		onAddButtonClick: function(button)
		{
			...
			for (i = 0; i < this.availableTriggers.length; ++i)
			{
				menuItems.push({
					...
					onclick: function(e, item)
					{
						me.addTrigger(item.triggerData);
			...
			if (trigger.data['CODE'] == 'WEBHOOK')
			{
				...
				BX.ajax({
					...
					onsuccess: function(response)
					{
						...
						{
							hookLinkTextarea.value = window.location.protocol + '//' + window.location.host + response['DATA']['HANDLER'];
						}
					}
				});
			}
	...
	Trigger.prototype =
	{
		init: function(data, viewMode)
		{
			...
			this.viewMode = viewMode || Component.ViewMode.View;
			this.node = this.createNode();
			this.manager.insertTriggerNode(this.getStatusId(), this.node);
		},
		reInit: function(data, viewMode)
		{
			var node = this.node;
	...
	DelayIntervalSelector.prototype =
	{
		...
		showDelayIntervalPopup: function()
		{
			...
			var labelAfter = BX.create("label", {
				...
				"<label class=\"crm-automation-popup-settings-link\" for=\"vt11"+uid+"\">"+BX.message('CRM_AUTOMATION_CMP_INTERVAL_M')+"</label>" +
				...
				"<label class=\"crm-automation-popup-settings-link\" for=\"vt12"+uid+"\">"+BX.message('CRM_AUTOMATION_CMP_INTERVAL_H')+"</label>" +
				...
				"<label class=\"crm-automation-popup-settings-link\" for=\"vt13"+uid+"\">"+BX.message('CRM_AUTOMATION_CMP_INTERVAL_D')+"</label>"
			});
			...
			if (this.basisFields.length > 0)
			{
				...
				var labelBefore = BX.create("label", {
					...
					+ (delay.valueType == 'i' || delay.type == DelayInterval.Type.After ? ' checked' : '') + ">" +
					"<label class=\"crm-automation-popup-settings-link\" for=\"vt21" + uid + "\">" + BX.message('CRM_AUTOMATION_CMP_INTERVAL_M') + "</label>" +
					...
					"<label class=\"crm-automation-popup-settings-link\" for=\"vt22" + uid + "\">" + BX.message('CRM_AUTOMATION_CMP_INTERVAL_H') + "</label>" +
					...
					"<label class=\"crm-automation-popup-settings-link\" for=\"vt23" + uid + "\">" + BX.message('CRM_AUTOMATION_CMP_INTERVAL_D') + "</label>" +
