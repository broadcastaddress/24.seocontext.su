...
if (!$bInternal)
{
	...
	if($arResult['CATEGORY_ID'] >= 0)
	{
		$arResult['FILTER'][] = array('id' => 'STAGE_ID', 'name' => GetMessage('CRM_COLUMN_STAGE_ID'), 'type' => 'list', 'items'=> DealCategory::getStageList($arResult['CATEGORY_ID']), 'params' => array('multiple' => 'Y'));
	}
	else
	{
		$arResult['FILTER'][] = array('id' => 'CATEGORY_ID', 'name' => GetMessage('CRM_COLUMN_CATEGORY_ID'), 'default' => true, 'type' => 'list', 'items' => DealCategory::getSelectListItems(), 'params' => array('multiple' => 'Y'));
		$arResult['FILTER'][] = DealCategory::getStageFilterInfo(array('id' => 'STAGE_ID', 'name' => GetMessage('CRM_COLUMN_STAGE_ID'), 'params' => array('multiple' => 'Y')));
	}

	$arResult['FILTER'][] = Bitrix\Crm\PhaseSemantics::getListFilterInfo(CCrmOwnerType::Deal, array('id' => 'STAGE_SEMANTIC_ID', 'name' => GetMessage('CRM_COLUMN_STAGE_SEMANTIC_ID'), 'params' => array('multiple' => 'Y')));
	$arResult['FILTER'] = array_merge(
		...
		array(
			...
			//array('id' => 'CLOSED', 'name' => GetMessage('CRM_COLUMN_CLOSED'), 'type' => 'listlist', 'items' => array('' => '', 'Y' => GetMessage('MAIN_YES'), 'N' => GetMessage('MAIN_NO'))),
	...
	$arResult['FILTER_PRESETS'] = array(
		'filter_new' => array(
			'name' => GetMessage('CRM_PRESET_NEW'),
			'fields' => array('IS_NEW' => 'Y')
		),
		'filter_my' => array(
			'name' => GetMessage('CRM_PRESET_MY'),
			'fields' => array( 'ASSIGNED_BY_ID_name' => $currentUserName, 'ASSIGNED_BY_ID' => $currentUserID)
		)
	);
}
...
$arResult['HEADERS'] = array(
	...
	array('id' => 'STAGE_ID', 'name' => GetMessage('CRM_COLUMN_STAGE_ID'), 'sort' => 'stage_sort', 'default' => true, 'editable' => false, 'type' => 'list')
);
...
$effectiveCategoryID = $arResult['CATEGORY_ID'] ? $arResult['CATEGORY_ID'] : -1;
if($effectiveCategoryID < 0 && isset($arFilter['CATEGORY_ID']))
{
...
$arImmutableFilters = array('FM', 'ID', 'ASSIGNED_BY_ID', 'CURRENCY_ID', 'CONTACT_ID', 'ASSOCIATED_CONTACT_ID', 'COMPANY_ID', 'STAGE_SEMANTIC_ID', 'CREATED_BY_ID', 'MODIFY_BY_ID', 'PRODUCT_ROW_PRODUCT_ID', 'WEBFORM_ID');
...
if (!$isInExportMode)
{
	$arResult['NEED_FOR_REBUILD_DEAL_ATTRS'] =
		$arResult['NEED_FOR_REBUILD_DEAL_SEMANTICS'] = false;
