...
<div id="appframe_layout_<?=$arResult['APP_SID']?>" <? if(!empty($frameStyle)) echo ' style="'.implode(';', $frameStyle).'"' ?> class="app-frame-layout">
	<iframe id="appframe_<?=$arResult['APP_SID']?>" name="<?=htmlspecialcharsbx($frameName)?>" frameborder="0" class="app-frame app-loading"></iframe>
...
?>
BX.rest.AppLayout.get('<?=$arResult['APP_SID']?>').expandPopup = BX.rest.AppLayout.get('<?=$arParams['PARENT_SID']?>').expandPopup;
