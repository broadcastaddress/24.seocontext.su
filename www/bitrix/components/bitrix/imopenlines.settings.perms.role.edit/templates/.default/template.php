						...
						foreach ($actions as $action => $availablePermissions)
						{
							?>
								<tr class="<?=($firstAction ? 'tr-first' : '')?>">
									<td class="table-blue-td-name"><?=($firstAction ? htmlspecialcharsbx(\Bitrix\ImOpenlines\Security\Permissions::getEntityName($entity)) : '&nbsp;')?></td>
									<td class="table-blue-td-param"><?=htmlspecialcharsbx(\Bitrix\ImOpenlines\Security\Permissions::getActionName($action))?></td>
