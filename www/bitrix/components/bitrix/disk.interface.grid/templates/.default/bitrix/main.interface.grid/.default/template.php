...
<?endif?>
		<td class="bx-disk-actions-col" width="1%"><a href="javascript:void(0);"
			...
			title="<?echo GetMessage("interface_grid_settings")?>" class="bx-disk-action"><div class="empty"></div></a></td>
<?
...
<?endif?>
	<?if(is_array($aRow["actions"]) && count($aRow["actions"]) > 0):?>
		<td class="bx-disk-actions-col"><a href="javascript:void(0);"
			...
			title="<?echo GetMessage("interface_grid_act")?>" class="bx-disk-action"><div class="empty"></div></a></td>
	<?else:?>
		<td>&nbsp;</td>
	<?endif?>
<?foreach($arResult["HEADERS"] as $id=>$header):?>
