function JSEntitySelectControl(arParams)
{
	var _this = this;
	this.arParams = arParams; // {LANGUAGE_ID, ONSELECT, MULTIPLE, VALUE, SITE_ID}
...
JSEntitySelectControl.prototype.OnSelect = function()
{
	if (null != this.arParams.ONSELECT)
	{
		...
		if (this.arParams.GET_FULL_INFO)
		{
			...
			else
			{
				...
				for (var i = 0; i < value.length; i++)
				{
					var v = this.arEntityData[value[i]];
...
JSEntitySelectControl.prototype.Show = function(arParams)
{
	if(null != this.div)
		return;
	...
	if (null == arParams.id) arParams.id = 'entity_select_control';
	...
	var url = '/bitrix/components/bitrix/crm.entity.search/'+this.arParams.ENTITY_TYPE + '.ajax.php?lang=' + this.arParams.LANGUAGE_ID + '&win_id=' +this.arParams.WIN.id + '&SITE_ID=' + this.arParams.SITE_ID;
...
JSEntitySelectControl.prototype._ShowWindow = function(result)
{
	CloseWaitWindow();
	...
	jsUtils.onCustomEvent('onEntitySearchShow', {div: this.div});
	
	BX.bind(document, "keypress", this._onkeypress);
}

JSEntitySelectControl.prototype.CloseDialog = function()
{
	BX.unbind(document, "keypress", this._onkeypress);
	
	jsUtils.onCustomEvent('onEntitySearchClose', {div: this.div});
